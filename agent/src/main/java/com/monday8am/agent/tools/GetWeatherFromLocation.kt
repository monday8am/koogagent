...
/**
 * Koog Tool for fetching weather information. This tool allows the AI agent to autonomously request
 * weather data when it determines weather context is needed for notification generation.
 */
@Serializable
...
class GetWeatherFromLocation(private val weatherProvider: WeatherProvider) :
    ...
    @Serializable
    data class Args(
        @property:LLMDescription("Latitude of the location in double format") val latitude: Double,
        @property:LLMDescription("Longitude of the location in double format") val longitude: Double,
    )

    /**
     * Fetches current weather information for the user's location. Use this tool when you need
     * weather context to personalize meal or hydration suggestions.
     */
