...
/**
 * Singleton to trace tool execution for testing purposes. This allows verifying if a tool was
 * called by the inference engine, even if we don't have access to the conversation history.
 */
object ToolTrace {
    ...
    fun log(toolCall: ToolCall) {
        synchronized(this) { _calls.add(toolCall) }
    }
